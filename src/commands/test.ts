...
export function createTestCommand(): Command {
  const command = new Command()
    ...
    .action(async (options) => {
      ...
      const { alias, pingOnly, verbose } = options;
      ...
      if (verbose) {
        console.log(chalk.gray(`Instance URL: ${config.instanceUrl}`));
        console.log(chalk.gray(`API Key: ${config.apiKey.substring(0, 4)}***${config.apiKey.substring(config.apiKey.length - 4)}`));
      }
